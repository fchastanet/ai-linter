...
class TestLogger:
    ...
    def test_logger_initialization_default(self) -> None:
        ...
        assert logger.formatter.get_format() == LogFormat.FILE_DIGEST
    ...
    def test_logger_initialization_with_params(self) -> None:
        ...
        assert logger.formatter.get_format() == LogFormat.YAML
    ...
    def test_log_filtering_by_level(self) -> None:
        ...
        logger.log(LogLevel.INFO, "info-rule", "Info message", "test.py", 10)
        logger.log(LogLevel.ERROR, "error-rule", "Error message", "test.py", 20)
    ...
    def test_log_buffering_for_file_digest(self) -> None:
        ...
        logger = Logger(format=LogFormat.FILE_DIGEST)
        logger.log(LogLevel.ERROR, "test-rule", "Test message", "test.py", 42)
        assert len(logger.messages) == 1
    ...
    def test_buffer_message_structure(self) -> None:
        ...
        logger = Logger(format=LogFormat.FILE_DIGEST)
        logger.log(LogLevel.WARNING, "test-rule", "Test message", "test.py", 42)
        assert len(logger.messages) == 1
